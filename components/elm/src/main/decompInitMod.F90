module decompInitMod
  ...
  public decompInit_lnd_uELM      ! initializes lnd grid decomposition into clumps and
                                  !processors using uELM schemes
...
contains
  ...
  !------------------------------------------------------------------------------
  subroutine decompInit_lnd_uELM(lni,lnj,amask)
    !
    ! !DESCRIPTION:
    ! This subroutine initializes the land surface decomposition into a clump
    ! data structure.  This assumes each pe has the same number of clumps
    ...
    integer :: ag,an,ai,aj            ! indices
    integer :: numg                   ! number of land gridcells
    integer :: ngridpc                ! number of land gridcells per clump
    logical :: seglen1                ! is segment length one
    real(r8):: seglen                 ! average segment length
    real(r8):: rcid                   ! real value of cid
    integer :: cid,pid                ! indices
    ...
    cid = 0
    do n = 1,nclumps
       pid = (n-1) / clump_pproc     ! distribute the gridcells based on clump_pproc
       if (pid < 0 .or. pid > npes-1) then
          write(iulog,*) 'decompInit_lnd_uELM(): uELM pid error ',n,pid,npes
          call endrun(msg=errMsg(__FILE__, __LINE__))
       endif
       clumps(n)%owner = pid
       if (iam == pid) then
          ...
          procinfo%cid(cid) = n
          write(iulog,*) 'n, cid, pid, clumps(n)%owner ',n,cid,pid, clumps(n)%owner
       endif
    enddo
    ...
    ! calculate the number of land gridcells in each clump (upper_bound)
    ngridpc = (numg-1)/nclumps + 1
   
    if (npes > numg) then
       write(iulog,*) 'decompInit_lnd_uELM(): Number of processes exceeds number ', &
            'of land grid cells',npes,numg
       call endrun(msg=errMsg(__FILE__, __LINE__))
    end if
    if (nclumps > numg) then
    ...
    allocate(lcid(lns), stat=ier)
    if (ier /= 0) then
       write(iulog,*) 'decompInit_lnd_uELM(): allocation error for lcid'
       call endrun(msg=errMsg(__FILE__, __LINE__))
    end if
    ...
    do ln = 1,lns
       ...
       !!      cid = mod(int(rcid),nclumps) + 1
       !!   endif
          cid = (ng-1)/ngridpc + 1     !   if each clump only contain 1 land gridcell, 
                                       !   so cid = ng   
          lcid(ln) = cid
          ...
          if (iam == clumps(cid)%owner) then
             procinfo%ncells  = procinfo%ncells  + 1
             write(iulog,*)'   iam, cid, clumps(cid)%owner, procinfo%ncells  = ', iam, cid, clumps(cid)%owner, procinfo%ncells
          ...
          write(iulog,*)'   iam, ln, cid, clumps(cid)%owner, clumps(cid)%ncells, procinfo%ncells  = ', &
                  iam, ln, cid, clumps(cid)%owner, clumps(cid)%ncells, procinfo%ncells
    ...
    allocate(ldecomp%gdc2glo(numg), stat=ier)
    if (ier /= 0) then
       write(iulog,*) 'decompInit_lnd_uELM(): allocation error1 for ldecomp, etc'
       call endrun(msg=errMsg(__FILE__, __LINE__))
    end if
    allocate(clumpcnt(nclumps),stat=ier)
    if (ier /= 0) then
       write(iulog,*) 'decompInit_lnd_uELM(): allocation error1 for clumpcnt'
       call endrun(msg=errMsg(__FILE__, __LINE__))
    end if
    ...
    if (masterproc) then
       ...
       write(iulog,*)' Decomposition Characteristics'
       write(iulog,*)'   clumps per process             = ',clump_pproc
       write(iulog,*)'   procinfo%begg, endg            = ',procinfo%begg, procinfo%endg
       write(iulog,*)' gsMap Characteristics'
       write(iulog,*) '  lnd gsmap glo num of segs      = ',mct_gsMap_ngseg(gsMap_lnd_gdc2glo)
       write(iulog,*)
    end if
  ...
  end subroutine decompInit_lnd_uELM
